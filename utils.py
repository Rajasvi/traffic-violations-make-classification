...
def pre_process(x,cat_cols):
    x["IssueHour"] = x["Issue time"].apply(lambda x: int(x)//100 if pd.notnull(x) else x,1).astype('category')

    x["IssueDateObj"] = pd.to_datetime(x["Issue Date"])
    x["IssueWeek"] = x["IssueDateObj"].dt.isocalendar().week
    x["IssueYear"] = x["IssueDateObj"].dt.year
    x["IssueWeekDay"] = x["IssueDateObj"].dt.weekday
    x["IssueDay"] = x["IssueDateObj"].dt.day
    x["00600_route"] = x["Route"].apply(lambda x: 1 if x=="0060" else 0,1).astype("category")
    x["CA_plate"] = x["RP State Plate"].apply(lambda x: 1 if x=="CA" else 0,1).astype("category")

    xn = x.drop(columns = ["IssueDateObj","Issue time","Issue Date"])

        
    for feature in ["Body Style","Route","Color","RP State Plate","Violation code"]:
        xn[feature] = xn[feature].mask(xn[feature].map(xn[feature].value_counts(normalize=True)) < 0.01, 'Other')
    
    for col in cat_cols:
        xn[col]= xn[col].astype('category')
